export default function SettingsLoading() {
  return (
    <div className="min-h-screen bg-[#f8fafc] select-none font-sans">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 md:px-8 mt-6 space-y-6 pb-20">
        {/* Header */}
        <div className="space-y-2">
          <div className="h-9 w-52 bg-slate-200 rounded-3xl animate-pulse" />
          <div className="h-4 w-64 bg-slate-200 rounded-full animate-pulse" />
        </div>

        {/* Section cards */}
        {[0, 1, 2].map((section) => (
          <div key={section} className="bg-white rounded-3xl p-6 border border-slate-100 space-y-5 shadow-[0_2px_15px_rgba(0,0,0,0.02)]">
            ...
            {[0, 1, 2].map((i) => (
              <div key={i} className="space-y-2">
                ...
                <div className="h-11 w-full bg-slate-200 rounded-2xl animate-pulse" />
              </div>
            ))}
